...
- **Alarms & Conditions missing subtypes/properties**: feature 095 closed `TransitionTime`/`EffectiveTransitionTime`/`EffectiveDisplayName` on the core sub-states (14 of the 58-CU range — the remaining 44 are per-substate `Effective*` beyond `ActiveState` and per-specific-transition-edge timestamps, a materially larger increment), all 10 Enable/Disable/Suppress/Unsuppress/RemoveFromService/PlaceInService/Silence Methods, A&C auditing for those Methods, and the Level-alarm `TypeDefinition` fix. Still open: Deviation alarms (`ExclusiveDeviationAlarmType`/`NonExclusiveDeviationAlarmType`, CUs `2390`/`2951`, needs a setpoint-tracking evaluator), RateOfChange alarms (CUs `2323`/`2946`, needs a rate-window evaluator), `SystemOffNormalAlarmType` (CU `2239`), `CertificateExpirationAlarmType` (CU `2236`, needs `ExpirationDate`/`ExpirationLimit`), `DiscrepancyAlarmType` (CU `2861`, needs `TargetValueNode` comparison), `OnDelay`/`OffDelay` timing (CU `2877`), `ReAlarmTime`/`ReAlarmRepeatCount` (CU `2879`), `AudibleSound`/`AudibleEnabled` (CU `2881`). See `specs/095-ac-completion/tasks.md` T045-T052 for grounded task breakdowns.
