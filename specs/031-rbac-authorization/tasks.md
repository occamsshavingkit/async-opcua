...
- [ ] T030 [US2] Resolve the role set at session activation and cache it on the Session; populate `RequestContextInner.user_roles` from it (Spec: Part 4 §5.6 ActivateSession; Part 18 §4)
...
- [ ] T095 [US8] Add `ServerBuilder` methods: `identity_mapping_rule`, `default_role_permissions`, `default_access_restrictions`, `enforce_role_based_access`, `with_secure_role_preset` `srv/server_builder` (Spec: Part 18 §4; Part 5 §6)
