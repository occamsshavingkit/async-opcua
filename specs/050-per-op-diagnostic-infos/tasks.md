...
- [ ] T002 [US1] Add the diagnostic slot to `BrowseNode` in `async-opcua-server/src/node_manager/view.rs`: `diagnostic_bits` + `diagnostic_info` fields, thread `diagnostic_bits` through `BrowseNode::new(...)`, add `diagnostic_bits()` getter + `set_diagnostic_info()` setter, and `impl IntoResult for BrowseNode { type Result = BrowseResult; .. }` returning `(self.<result>, self.diagnostic_info)`.
- [ ] T003 [US1] Route Browse and BrowseNext through the shared emit in `async-opcua-server/src/session/services/view.rs`: pass `request.request_header.return_diagnostics` into each `BrowseNode::new(...)` call site, and replace the `diagnostic_infos: None` in the `BrowseResponse`/`BrowseNextResponse` construction with the array from `consume_results(nodes, return_diagnostics)`. Continuation-point logic unchanged.
- [ ] T004 [P] [US1] Independent test in `async-opcua-server/tests/per_op_diagnostics.rs` (new file): for Browse and BrowseNext, assert that with an operational `return_diagnostics` bit set the response `diagnostic_infos` is `Some` with `len() == results.len()`, and with `DiagnosticBits::empty()` it is `None`. Use raw `UARequest` builders. Red-first (Browse case fails before T002/T003).
...
- [ ] T005 [US2] Add the diagnostic slot + `IntoResult` to `MonitoredItemRef` (result: create→`MonitoredItemCreateResult`; delete/set-mode→`StatusCode`) in `async-opcua-server/src/node_manager/monitored_items.rs`, threading `diagnostic_bits` through its `new(...)`.
- [ ] T006 [US2] Add the diagnostic slot + `IntoResult` to `MonitoredItemUpdateRef` (result: `MonitoredItemModifyResult`) in `async-opcua-server/src/node_manager/monitored_items.rs`, threading `diagnostic_bits` through its `new(...)`.
- [ ] T007a [US2] Route CreateMonitoredItems and DeleteMonitoredItems in `async-opcua-server/src/session/services/monitored_items.rs` through `consume_results(items, return_diagnostics)`, replacing their `n: None` / `diagnostic_infos: None` (Create→`MonitoredItemCreateResult`, Delete→`StatusCode`).
- [ ] T007b [US2] Route ModifyMonitoredItems in `async-opcua-server/src/session/services/monitored_items.rs` through `consume_results(items, return_diagnostics)`, replacing its `diagnostic_infos: None` (result `MonitoredItemModifyResult`).
- [ ] T007c [US2] Route SetMonitoringMode in `async-opcua-server/src/session/services/monitored_items.rs` through `consume_results(items, return_diagnostics)`, replacing its `diagnostic_infos: None` (result `StatusCode`).
- [ ] T007d [US2] Route SetTriggering in `async-opcua-server/src/session/services/monitored_items.rs` through the shared emit for BOTH its add and remove arrays (`add_diagnostic_infos` + `remove_diagnostic_infos`), replacing each `n: None` / `diagnostic_infos: None`.
...
- [ ] T014b [P] [US4] Independent test in `async-opcua-server/tests/per_op_diagnostics.rs` for QueryNext: obtain a QueryFirst continuation point, then assert QueryNext's operation-level `diagnostic_infos` (and nested `data_diagnostic_infos` where a data set is returned) is aligned+present when requested and `None` when not. If QueryNext cannot be driven to a per-op result in the test harness, document the exact reason inline and cover the QueryNext emit path via the shared `consume_results` call site assertion instead (do NOT silently drop coverage).
