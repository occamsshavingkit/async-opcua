...
- [X] T012 [US4] *(revised per T001 reasoning)* NO slot on `QueryRequest`: the op-level array aligns to `node_types` (one `ParsingResult` each, per Part 4 B.2.3 "diagnostic information for the requested NodeTypeDescription"), but node managers mutate the whole `QueryRequest`, so a single diagnostic field has no operation to align with. Instead thread `DiagnosticBits` into `ParsedNodeTypeDescription::parse` in `async-opcua-server/src/node_manager/query.rs` and gate the nested level there: when bits non-empty and `data_status_codes` is `Some(v)`, set `data_diagnostic_infos = Some(vec![DiagnosticInfo::default(); v.len()])` (aligned to dataStatusCodes); `None` otherwise.
- [X] T013 [US4] Route QueryFirst in `async-opcua-server/src/session/services/query.rs` through the shared emit for the operation-level array on ALL THREE response paths (bad-parse early return, node-manager-error return, success): pairs `(ParsingResult, None)` into `consume_results(pairs, return_diagnostics)` (success path keeps `parsing_results: None` but still emits the aligned `diagnostic_infos` from the locally built parsing results). Pass the header bits into each `ParsedNodeTypeDescription::parse` call. QueryNext unchanged (no wire field — T014b).
- [X] T014 [P] [US4] Independent test in `async-opcua-server/tests/per_op_diagnostics.rs` for QueryFirst: assert (a) the **operation-level** `diagnostic_infos` is aligned+present when requested and `None` when not, and (b) the **nested** `ParsingResult.data_diagnostic_infos` follows the same bits-gated rule (aligned to `data_status_codes` when requested, `None` when not). Raw `UARequest` builders. Red-first.
