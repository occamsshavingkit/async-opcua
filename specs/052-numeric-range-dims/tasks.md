...
- [ ] T201 Verify current shape unchanged (numeric_range.rs:227 cap; from_ua_string Invalid
  fallback; per-dimension consumers) — done during research 2026-07-02; re-confirm on branch.

## Phase 2: US1 — cap removal (P1)

- [ ] T202 [US1] Claude: red-first tests. In `async-opcua-types/src/numeric_range.rs` tests: move
...
- [ ] T203 [US1] codex: in `async-opcua-types/src/numeric_range.rs`, remove `MAX_INDICES` and the
...
- [ ] T204 Update `specs/complexity-cuts-backlog.md` `Variant::range_of` entry: stale — resolved by
  feature 017's per-dimension rework; cap removed by 052 without reintroducing O(n·m).
- [ ] T205 FINDINGS.md P4-ATTR-06 → FIXED (with corrected behavior description: Invalid-fallback →
  per-op BadIndexRangeInvalid, not decode failure).
- [ ] T206 Full gate: `cargo test -p async-opcua-types`, full workspace suite via
