...
- [X] T009 [US1] Structural proof test (load-bearing, C6) in `async-opcua-server/tests/`: an OSC handshake on `#[tokio::test(flavor = "current_thread")]` while the single worker is kept busy — it can only complete if the crypto runs on the blocking pool. Distinguishes "actually offloaded" from "merely refactored".
