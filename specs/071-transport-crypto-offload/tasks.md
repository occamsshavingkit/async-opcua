...
- [X] T004 [P] Unit tests for both cores in `async-opcua-core/src/comms/secure_channel.rs` (`#[cfg(test)]`): sign→verify / encrypt→decrypt round-trip per policy, and route the existing secure-channel crypto regression vectors through the free functions to prove G2 equivalence (identical output to the `&self` methods).
...
- [X] T005 [US1] Site 1 (the DoS lever, **pre-auth**): offload the server inbound OSC decrypt+verify onto `spawn_blocking` in `async-opcua-server/src/transport/tcp.rs` — the `SecureChannel::verify_and_remove_security_server` (`secure_channel.rs:1220`) path calling `asymmetric_decrypt_and_verify_owned`; extract owned inputs (brief borrow, no lock across `.await`), unwrap the inner crypto `Result` first, and add a distinct `JoinError` arm that drops the connection with a transport fault without masking a specific crypto fault (C2/R6). OPC-10000-4 §5.6.2 (OpenSecureChannel Service); message security OPC-10000-6 §6.7.2.
- [X] T006 [US1] Site 2: offload the server outbound OSC sign+encrypt onto `spawn_blocking` at the OSC-encode seam — `SendBuffer::encode_next_chunk` (`async-opcua-core/src/comms/buffer.rs:164`) async path driven by `async-opcua-server/src/transport/tcp.rs` send; the transport MUST `.await` the offloaded chunk before emitting the next, preserving the monotonic per-`MessageChunk` sequence number (OPC-10000-6 §6.7.2.4 Sequence Header; C3/R5). No dedicated reorder test is needed — OSC is a single `MessageChunk` per channel establishment, so reordering is structurally impossible; the existing chunk/sequence tests guard the general path.
- [X] T007 [US1] Site 4a: offload CreateSession server-signature RSA signing onto `spawn_blocking` in `async-opcua-server/src/session/controller.rs` (`CreateSessionDraft::prepare_endpoint_preflight` ~:565 — already outside the session-manager write lock). OPC-10000-4 §5.7.2 (CreateSession Service — serverSignature over clientCertificate + clientNonce).
- [X] T008 [US1] Site 4b: offload ECC ephemeral-key generation onto `spawn_blocking` in `async-opcua-server/src/session/manager.rs` (`issue_server_ephemeral_key` ~:357 and the renew site ~:1539). OPC-10000-6 §6.8.2 (ECC EphemeralKey returned in the CreateSession response for UserIdentityToken encryption) + OPC-10000-4 §5.7.2 (CreateSession Service) / §7.15 (EphemeralKeyType).

### Tests for User Story 1

- [X] T009 [US1] Structural proof test (load-bearing, C6) in `async-opcua-server/tests/`: an OSC handshake on `#[tokio::test(flavor = "current_thread")]` while the single worker is kept busy — it can only complete if the crypto runs off the async worker (dedicated executor after T010A, `spawn_blocking` fallback otherwise). Distinguishes "actually offloaded" from "merely refactored".
- [X] T010 [US1] `#[ignore]`'d 50-client handshake-storm test (SC-001) in `async-opcua-server/tests/`: ≥50 concurrent channel-openers while one established session issues reads; assert the established session's p99 read latency **under the storm is ≤ 2× its p99 measured with no concurrent handshakes** — a fixed relative bound (not an absolute latency), so it cannot scale with handshake count. Manual, `taskset -c <core>` per repo benchmarking convention.
- [X] T010A [US1] Priority-aware server asymmetric crypto executor: introduce a single bounded, lower-priority dedicated worker lane (a size-limited separate pool / dedicated runtime worker set with reduced scheduling priority — not the shared default `spawn_blocking` pool) for the server-side handshake crypto, and rewire the four existing US1 offload sites to dispatch through it: the OSC inbound decrypt+verify (T005, `async-opcua-server/src/transport/tcp.rs`), the OSC outbound sign+encrypt (T006, `SendBuffer::encode_next_chunk`), the CreateSession server-signature RSA sign (T007, `controller.rs`), and the ECC ephemeral-key generation (T008, `manager.rs`). Rationale (from T010): under a handshake storm the shared blocking pool's RSA threads contend with established-session reads on the runtime workers; perf attributes the CPU to RSA modular arithmetic (`num_bigint_dig` Montgomery / `aws_lc` BN-RSA), not app lock contention, so the fix is scheduling priority, not more parallelism. The lane MUST be bounded (cap concurrent crypto workers; queue/backpressure rather than unbounded thread spawn) and MUST NOT introduce new locks, mutexes, or blocking on the symmetric per-request hot path (C5/FR-007, AGENTS.md async invariant). Wire bytes, chunk sequence numbers, and StatusCodes stay identical (OPC-10000-6 §6.7.2 message security; OPC-10000-4 §5.6.2 OpenSecureChannel Service; OPC-10000-4 §5.7.2 CreateSession Service; OPC-10000-6 §6.8.2 ECC ephemeral key returned in CreateSession response) — only *where* the crypto is scheduled changes. Validate by re-running T009 (structural proof — handshake still completes on a starved `current_thread` worker) and T010 (storm test — established-session p99 read latency under the storm holds the ≤ 2× no-storm bound, demonstrating the priority separation actually moves RSA off the contended cores).
...
- **US1 (T005–T010A)**: after Foundational. T005–T008 are the four offload sites (largely independent files: tcp.rs, buffer.rs+tcp.rs, controller.rs, manager.rs — but T005/T006 both touch tcp.rs → sequential). Tests T009/T010 after the sites they exercise. T010A (priority-aware executor) after T005–T008 (rewires their dispatch sites) and validated by re-running T009/T010 — it is the response to T010's storm findings.
