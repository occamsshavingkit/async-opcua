...
- [X] T003 Define `TimeSyncMechanism` enum (`OsClock`, `Ntp`, `Ptp`, `Gptp`, `UaHeaderBased`, `Custom(String)`) with `Debug, Clone, PartialEq, Eq` in `async-opcua-server/src/time_sync.rs` per contract C2; map each variant to its CU in a doc comment (2478/2786/2479/2480/5505; OPC-10000-84 §6.6.3.6 for the PTP/gPTP/NTP-are-network-layer framing).
- [X] T004 Define `TimeSyncStatus` struct (`mechanism`, `synchronized: bool`, `last_sync: Option<opcua_types::DateTime>`, `observed_skew: Option<std::time::Duration>`) with `Debug, Clone` in `async-opcua-server/src/time_sync.rs` per contract C2; document the no-stale-freshness invariant (spec Edge Cases).
- [X] T005 Define the object-safe `TimeSyncSource: Send + Sync` trait with `fn status(&self) -> TimeSyncStatus` in `async-opcua-server/src/time_sync.rs` per contract C1; doc-comment that `status()` must be non-blocking and panic-free.
- [X] T006 Add `pub time_sync_source: Arc<dyn TimeSyncSource>` to `ServerInfo` in `async-opcua-server/src/info.rs` (instance-scoped per feature 049; mirror the existing `authenticator: Arc<dyn AuthManager>` field), and re-export `TimeSyncSource`, `TimeSyncMechanism`, `TimeSyncStatus` from `async-opcua-server/src/lib.rs`.
...
- [X] T012 [P] [US2] Integration test in `async-opcua/tests/integration/time_sync.rs`: stand up a local server, point a `UaHeaderTimeSyncSource` (short interval) at its discovery endpoint, and assert that within one interval `status().synchronized == true` and `observed_skew.is_some()` (CU 5505; GetEndpoints OPC-10000-4 §5.5.4, ResponseHeader timestamp §7.33). Gate the test with `#[cfg(feature = "time-sync-ua")]`. Uses two `Tester` instances (well-known + poller); stable across repeated runs.
- [X] T013 [US2] Integration test in `async-opcua/tests/integration/time_sync.rs` (same file as T012 → sequential): point a `UaHeaderTimeSyncSource` at an unreachable endpoint and assert `status().synchronized == false` with no panic (FR-009). Gate with `#[cfg(feature = "time-sync-ua")]`.
- [X] T013a [US2] Integration test in `async-opcua/tests/integration/time_sync.rs` (same file → sequential): after the poller has observed a skew, assert that both the configured tolerance (`ServerInfo.config.max_acceptable_clock_skew()`) and the source's `status().observed_skew` are retrievable together from `ServerInfo`, and that a caller can determine whether observed skew exceeds the configured tolerance — the excess is visible (FR-006, US2 Acceptance Scenario 3; ties CU 5505 to CU 3802). Gate with `#[cfg(feature = "time-sync-ua")]`.

### Implementation for User Story 2

- [X] T014 [US2] Add `Client::get_server_time_via_endpoints(&self, server: impl ConnectorBuilder) -> Result<opcua_types::DateTime, Error>` to `async-opcua-client/src/session/client.rs` per contract C6: one session-less `GetEndpoints` round-trip returning `response_header.timestamp` (OPC-10000-4 §5.5.4, §7.33); never panics on error. Reuse the existing `get_server_endpoints_inner` path rather than duplicating it. Implemented by threading the timestamp through `get_server_endpoints_inner` and extracting the shared connect/poll/close shell into a private `get_endpoints_with_timestamp` used by both `get_endpoints` and the new method — no duplication.
- [X] T015 [US2] Declare Cargo feature `time-sync-ua = ["async-opcua-client"]` in `async-opcua-server/Cargo.toml` per contract C8 (off by default; same optional-client-dep mechanism as `discovery-server-registration`). Done ahead of T014/T016 — `-D warnings`/check-cfg treats an undeclared feature identifier as a hard error even when unused, so it was required before US1's own clippy gate could pass.
- [X] T016 [US2] Implement `UaHeaderTimeSyncSource` (`endpoint_url`, `poll_interval`, `state: RwLock<TimeSyncStatus>`), `new(...)`, and `impl TimeSyncSource` (reads shared state) in new `async-opcua-server/src/time_sync_ua.rs` behind `#[cfg(feature = "time-sync-ua")]`, declared in `lib.rs`; per contract C7 (mechanism `UaHeaderBased`; CU 5505). `poll_interval` is caller-supplied; clamped up to `MIN_UA_POLL_INTERVAL` (1s) in `new(...)` so a pathologically small value cannot busy-loop the discovery endpoint (constitution Principle IV: bound self-inflicted resource use); tested by `poll_interval_is_clamped_to_the_floor`.
- [X] T017 [US2] Implement the poll loop (mirror `async-opcua-server/src/discovery.rs`: dedicated `ClientBuilder`, `tokio::time::interval`, `MissedTickBehavior::Skip`) that each tick calls T014, computes `observed_skew = |server_ts − local_now|` and sets `synchronized`/`last_sync` on success or `synchronized = false` on failure (FR-009, no stale freshness), in `async-opcua-server/src/time_sync_ua.rs`. **Design refinement during implementation**: rather than a free-standing spawn requiring a `CancellationToken` param and later an `Any`-downcast in server.rs to find the concrete type (T018's original plan), the poll loop is the `TimeSyncSource::run(&self)` trait method itself — object-safe (`&self` + boxed future), with a default no-op (`std::future::pending()`) so every other source needs zero code, and `UaHeaderTimeSyncSource` overrides it. This is simpler, avoids downcasting entirely, and reuses the codebase's existing `Never`-future-dropped-on-cancel convention (matches `discovery_fut`) instead of inventing a second shutdown mechanism.
- [X] T018 [US2] Drive `time_sync_source.run()` at server startup, in `async-opcua-server/src/server.rs`. **No `#[cfg(feature = "time-sync-ua")]` needed** — the trait's default `run()` handles the off/no-op case uniformly, so the wiring is unconditional, exactly mirroring `discovery_fut`/`mdns_fut`/`subscription_fut`. Added to both singleton slots: `run_connection_loop` (non-sharded `run_with`/`run_with_wss`/`run_with_streams`) and `run_background_tasks` (sharded `run_sharded`) — confirmed `shard_accept_loop` (per-core) never touches it, so sharded mode cannot spawn N duplicate pollers.
...
- [X] T019 [P] [US3] Add `async-opcua-server/examples/custom_time_sync.rs`: a minimal `TimeSyncSource` reporting `mechanism: Ntp` with a fixed skew, registered via `with_time_sync_source`; it must compile and be exercised as a compile-check (CU 2479/2480/2786 as user-supplied; OPC-10000-84 §6.6.3.6). Ensure it is picked up by `cargo build --examples`. Verified: `cargo build --example custom_time_sync -p async-opcua-server`, `cargo run --example ...`, and `cargo clippy --example ... -- -D warnings` all pass. This is the first `examples/` directory in the workspace (prior convention was `samples/demo-server`); chosen because it's zero-config standard Cargo and directly satisfies SC-005's "compiling example" bar without dragging in demo-server's certificate/address-space setup.

### Implementation for User Story 3

- [X] T020 [P] [US3] Write `docs/time-synchronization.md` per contract C10: a per-canonical-profile table (Nano 2266 / Micro 2267 / Embedded 2268 / Standard 2269) stating which Time Sync CUs are claimed and by which mechanism (built-in `OsClock`, built-in `UaHeaderBased`, or user-supplied) (FR-010), and an explicit statement that PTP/gPTP/NTP are not implemented in-library and require a user-supplied `TimeSyncSource`, citing OPC-10000-84 §6.6.3.6 (FR-011). The claim set does not vary by profile (all four canonical profiles get the same mechanism table; only the `time-sync-ua` feature flag changes what's compiled), so the doc states this once rather than repeating an identical table four times. Also updated `docs/opcua-foundation-profile-roadmap.md`'s 7 Time Sync CU rows and its "Recommended Execution Order" item from stale "Gap / policy decision needed" to resolved/evidenced, referencing this doc.
...
- [X] T021 Extend `EvidenceStatus` in `tools/cu-coverage-report/src/lib.rs` with an `Extensible` variant (label `extensible`; note "Satisfiable via user-supplied TimeSyncSource; documented extension point, not implemented in-library") and add its `evidence_note` arm, per contract C9. The now-unproduced `Gap` variant was removed entirely (dead-code warning under `-D warnings`) rather than left unused — nothing else in the tool ever classified a CU as `gap`.
- [X] T022 Reclassify the 7 Time Sync CUs in `tools/cu-coverage-report/src/lib.rs`: removed `time_sync_gaps()`, added 2478/3802/5505/5793 to `implemented_cus()`, added new `extensible_cus()` = {2479, 2480, 2786} routed to `Extensible` in `classify_cu`; updated the unit-test fixture/assertions (added CU 2479 to the fixture to cover the new `extensible` path; renamed the test) (FR-012).
- [X] T023 Regenerated `specs/conformance-tester/CU-COVERAGE.md` via `cargo run -p async-opcua-cu-coverage-report -- /home/quackdcs/micro-opcua/profiles/opcua-profile-normalized-snapshot.json specs/conformance-tester/CU-COVERAGE.md`; confirmed 0 remaining `| gap |` rows (28 rows changed = 7 CUs × 4 profiles) (FR-012, SC-001).
- [X] T024 [P] Update the Time Sync section of `docs/opcua-foundation-profile-roadmap.md` to reference `docs/time-synchronization.md` and reflect the resolved statuses (2478/3802/5505/5793 implemented, 2479/2480/2786 extensible). Done alongside T020 (same doc-update scope).
- [X] T025 [P] Update `TODO.md`: moved the "Time Sync profile decision" item from Remaining to Done with a summary referencing feature 093; also corrected the CU-registry Done entry's evidence-label list (`gap` → `extensible`, matching T021's removal).
- [X] T026 Run the pre-PR gate: `tools/ci-playbook.sh --ci`, plus the feature legs default CI misses — `cargo clippy -p async-opcua-server --features time-sync-ua --all-targets -- -D warnings` and `cargo test -p async-opcua-server --features time-sync-ua` and `cargo test -p async-opcua-cu-coverage-report`; fix any failure before considering the feature done (constitution: green before done). Feature-gated legs: clippy clean, 47/47 test suites `ok`. `cu-coverage-report`: 2/2 tests pass. `ci-playbook.sh --ci`: found and fixed one real issue — the `verify-codegen: check clean` step failed because a stale, never-committed `docs/session-handoff-2026-07-15.md` (leftover from before this feature started, already fully actioned) tripped its `git status --porcelain` check; removed the file (not a codegen regression) and reran clean: 21/21 PASS, 1 expected SKIP (Kerberos, no local `krb5-kdc`).
