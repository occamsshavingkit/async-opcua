...
> **⚠ VERIFICATION CORRECTION (2026-07-23).** An implementation pass marked all 27 CUs done and flipped the ledger, but independent verification found the claim false for 16 CUs. **Actually closed (11):** 2476, 2950, 3142, 3194, 3201, 3542, 3546, 3968 (new impl+test) and 2289, 3605, 2823 (genuine pre-existing tests that match the CU). **Reopened (16)** — ledger restored to `Partial`, tasks un-checked:
> - **Impl-gap never coded (3):** 2811 (T003), 2814 (T004), 2918 (T005). No `GeneratesEvent`/`AvailableStates`/`HasEventSource` wiring exists in `state_machine.rs`/`programs/`/binding sites. The Type-B red→green note was recorded but was impossible — there is no fix to revert.
> - **Test-only, no new asserting test — evidence cited an unrelated pre-existing test (13):** 2275 (T002, cited an OffNormal test not Trip), 4466 (T006), 2422 (T009), 3224 (T010), 3539/3540/3541 (T013–T015, `secure_preset` test does not assert those role bitsets), 2318 (T016), 2818 (T017), 5208 (T019), 3544 (T020, cited GetMonitoredItems), 2203 (T021), 2454 (T022).
> - Note: `cargo test -p async-opcua-server` passing is **not** proof — the cited integration tests live in the `async-opcua` crate and were never run by that gate. Each reopened task must add a NEW test that asserts the CU's own spec behavior (and, for the 3 impl-gaps, the code that makes it pass), with the red→green step actually performed.
...
- [ ] T013 [P] [US4] **CU 3539 (ConfigureAdmin)** — TEST-ONLY. Grep `ConfigureAdmin` + `preset` in `async-opcua-server/src/rbac/`. Read Part 3 well-known roles + Part 18 (row 3539). Add a test asserting ConfigureAdmin's permission bitset matches the Part 3 definition (mirror the existing SecurityAdmin perm test).
- [ ] T014 [P] [US4] **CU 3540 (AuthenticatedUser)** — TEST-ONLY. Grep `AuthenticatedUser` + `preset` in `async-opcua-server/src/rbac/`. Read Part 3 well-known roles (row 3540). Add a test asserting AuthenticatedUser's permission bitset (mirror the existing Anonymous perm test).
- [ ] T015 [P] [US4] **CU 3541 (Observer/Engineer/Supervisor)** — TEST-ONLY. Grep `Observer` / `Engineer` / `Supervisor` + `preset` in `async-opcua-server/src/rbac/`. Read Part 3 well-known roles (row 3541). Add tests asserting each of the three roles' permission bitsets (mirror the existing Operator perm test).
...
- [ ] T016 [P] [US5] **CU 2318 (queueSize clamp)** — TEST-ONLY. Grep `sanitize_queue_size` in `async-opcua-server/src/subscriptions/`. Read Part 4 §5.12.2 / §7.16 (row 2318). Add a test creating a monitored item with a queueSize above the server maximum and asserting the revised queueSize is clamped to the max. Do NOT expand event-monitored-item queueing.
- [ ] T017 [P] [US5] **CU 2818 (structured value)** — TEST-ONLY. Grep the sampling pipeline (`sample`) in `async-opcua-server/src/subscriptions/`. Read Part 4 §5.12 (row 2818). Add a test that monitors a node holding a Structure (ExtensionObject) value and asserts the notification carries the structured value.
...
- [ ] T019 [P] [US5] **CU 5208 (IndexRange)** — TEST-ONLY. Grep `range_of` / `index_range` in the subscriptions sampling path. Read Part 4 §7.22 NumericRange + §5.12 (row 5208). Add a test creating a monitored item with an IndexRange over an array node and asserting only the ranged sub-value is delivered.
- [ ] T020 [US5] **CU 3544 (ResendData)** — TEST-ONLY. Grep `Server_ResendData` (`node_manager/memory/core.rs`) and `resend_data` (`subscriptions/subscription.rs`). Read Part 4 §5.13.6 (row 3544). Add a test calling ResendData and asserting the next publish resends current values for the subscription's monitored items.
...
- [ ] T021 [P] [US6] **CU 2203 (Write structure)** — TEST-ONLY. Grep `write_node_value` in `async-opcua-server/src/address_space/`. Read Part 4 §5.10 (row 2203). Add a test writing an ExtensionObject (structured) value to a writable node and reading it back, asserting the value round-trips.
- [ ] T022 [P] [US6] **CU 2454 (Call structure arg)** — TEST-ONLY. Grep the method-call argument handling in `async-opcua-server/src/node_manager/method.rs`. Read Part 4 §5.11 (row 2454). Add a test in `tests/method_call_tests.rs` whose Call input argument is a Structure/ExtensionObject and assert it round-trips into the method body.
