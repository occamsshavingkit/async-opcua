...
> | P4-GEN-04 | NEW (found by 050's grep gate) | Per-op/nested diagnostics still hardcoded `None` at sites the audit never enumerated: TranslateBrowsePaths (`services/view.rs`), DeleteSubscriptions (`services/subscriptions.rs`), SetPublishingMode + Publish acks (`session_subscriptions.rs`), TransferSubscriptions (`subscriptions/mod.rs`), ActivateSession swCert results (`session/manager.rs`), RegisterServer2 configurationResults (`controller.rs`); nested: `CallMethodResult.input_argument_diagnostic_infos` (`node_manager/method.rs`), `HistoryUpdateResult.diagnostic_infos` (`node_manager/history.rs`), `EventFilterResult.select_clause_diagnostic_infos` (`services/subscription/filter.rs`). Same mechanical pattern as 050 — candidate follow-up feature |
