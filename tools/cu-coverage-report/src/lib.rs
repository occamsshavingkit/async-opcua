...
static AUDIT_TABLE: &[(u32, EvidenceStatus, &str)] = &[
    ...
    (2231, EvidenceStatus::Partial, "Feature 101: fixed a real bug (push_methods.rs registered callbacks against fabricated NodeIds resolving to unrelated A&C alarm properties, on the wrong node manager type -- ServerConfiguration is namespace-0/CoreNodeManager, not SimpleNodeManager) and removed a misplaced Pull-model method. Real ServerConfigurationType handlers now wired to verified NodeIds: CreateSigningRequest (ns=0;i=12737, real PKCS#10 CSR via x509_cert::builder::RequestBuilder), UpdateCertificate (13737, staged transaction), ApplyChanges (12740, commits via gds_reload::save_new_credentials+reload), CancelChanges (25708), GetRejectedList (12777), ResetToServerDefaults (25709); e2e-proven via gds_push_integration.rs (real Call service dispatch reaches the handler) + 15 unit tests in push_methods.rs. CreateSelfSignedCertificate/DeleteCertificate/GetCertificates confirmed absent from the generated nodeset (SourceIssue, not implementable). Still Partial: TrustListType (Open/OpenWithMasks/Read/CloseAndUpdate/AddCertificate/RemoveCertificate) and CertificateGroupType.GetRejectedList are a deferred follow-up run (see specs/101-gds-push-fix/). Sibling bug found in pull_methods.rs (CU 2230, same fabricated-NodeId pattern) -- out of scope, recorded in TODO.md"),
