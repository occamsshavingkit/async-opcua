...
static AUDIT_TABLE: &[(u32, EvidenceStatus, &str)] = &[
    ...
    (2203, EvidenceStatus::Implemented, "tests/integration/write_service.rs write_extension_object_round_trips_through_read_service"),
    ...
    (2236, EvidenceStatus::Implemented, "CertificateExpirationAlarm (alarms/certificate_expiration.rs) evaluates ExpirationDate/ExpirationLimit, register_certificate_expiration_alarm; test alarms.rs::certificate_expiration_alarm_activates_within_expiration_limit_and_clears_on_renewal"),
    (2239, EvidenceStatus::Implemented, "DiscreteAlarmKind::SystemOffNormal (discrete.rs) parameterizes DiscreteAlarm to report SystemOffNormalAlarmType, same OffNormalAlarmType evaluation; test alarms.rs::system_off_normal_alarm_reports_type_definition_and_activates"),
    ...
    (2275, EvidenceStatus::Implemented, "alarms/discrete.rs:199 trip_alarm_activates_returns_to_normal_and_has_trip_alarm_type_definition"),
    ...
    (2289, EvidenceStatus::Implemented, "Feature 109 CU 2289: event_history.rs:202-211 UpdateEvent upsert; tests history_events_inmemory.rs update_event_insert_and_read_back + sqlite history_events.rs update_event_insert_replace_and_read"),
    ...
    (2318, EvidenceStatus::Implemented, "subscriptions/monitored_item.rs sanitize_queue_size_clamps_to_server_maximum"),
    ...
    (2323, EvidenceStatus::Implemented, "RateOfChangeAlarm (alarms/rate_of_change.rs) reuses LimitAlarm evaluator against a computed per-second rate, register_rate_of_change_alarm; test alarms.rs::rate_of_change_alarm_reports_type_definition_and_activates_on_fast_change"),
    ...
    (2361, EvidenceStatus::Implemented, "create_two_state_discrete_variable (data_access.rs) instantiates TwoStateDiscreteType with mandatory TrueState/FalseState; test data_access.rs::two_state_discrete_exposes_true_false_states_and_value"),
    ...
    (2390, EvidenceStatus::Implemented, "DeviationAlarm (alarms/deviation.rs) reuses LimitAlarm evaluator against processValue-setpointValue for the non-exclusive path, register_deviation_alarm; test alarms.rs::deviation_alarm_reports_deviation_type_definition_and_activates_on_setpoint_deviation"),
    ...
    (2422, EvidenceStatus::Implemented, "tests/security_tests.rs audit_over_encrypted_channel"),
    ...
    (2426, EvidenceStatus::Implemented, "DiscreteItemType is abstract (OPC-10000-8 §5.3.3.1, 'no instances of this type can exist'); satisfied by any concrete subtype -- TwoStateDiscreteType/MultiStateDiscreteType/MultiStateValueDiscreteType (data_access.rs), tested in data_access.rs"),
    ...
    (2454, EvidenceStatus::Implemented, "tests/method_call_tests.rs custom_in_memory_node_manager_default_method_call_round_trips_extension_object_argument"),
    ...
    (2476, EvidenceStatus::Implemented, "Feature 109 CU 2476: node_manager/memory/core.rs:989-997 computes real TimeZoneDataType via chrono; test async-opcua-server/tests/server_local_time.rs test_server_local_time_returns_plausible_timezone"),
    ...
    (2711, EvidenceStatus::Implemented, "base_info::create_selection_list_variable instantiates SelectionListType with Selections/SelectionDescriptions/RestrictToList; test base_info.rs::selection_list_exposes_selections_descriptions_and_restrict_flag"),
    ...
    (2776, EvidenceStatus::Gap, "Depends on MultiStateDictionaryEntryDiscreteBaseType (2474, deferred -- see that entry); the property node exists in generated nodeset (ns=0;i=19083) but same undocumented-semantics blocker applies."),
    ...
    (2811, EvidenceStatus::Implemented, "alarms/state_machine.rs:537-541 GeneratesEvent→TransitionEventType + test L1564; programs/methods.rs:332-336 GeneratesEvent→ProgramTransitionEventType + test L588"),
    (2813, EvidenceStatus::Gap, "Searched 'AvailableStates'/'AvailableTransitions' - zero hits outside generated type def."),
    (2814, EvidenceStatus::Implemented, "alarms/state_machine.rs:262-308 AvailableStates/Transitions + test L1589; programs/methods.rs:262-308 + test L622"),
    ...
    (2818, EvidenceStatus::Implemented, "subscriptions/monitored_item.rs sampled_extension_object_reaches_subscription_notification"),
    (2820, EvidenceStatus::Implemented, "validate_node_write_inner (address_space/write_validation.rs) rejects an IndexRange Write to AttributeId::Value with Bad_WriteNotSupported when AccessLevelExType::WriteFullArrayOnly is set; test write.rs::write_index_range_rejected_when_write_full_array_only"),
    ...
    (2823, EvidenceStatus::Implemented, "Feature 109 CU 2823: Fixed 100ms tarpit on auth failure (session/negotiate.rs:16,28-40); test async-opcua-server/tests/security_tests.rs invalid_oauth2_jwt_validation_failure_is_tarpitted + username_password_auth_failure_is_tarpitted"),
    (2831, EvidenceStatus::Implemented, "create_multi_state_value_discrete_variable (data_access.rs) instantiates MultiStateValueDiscreteType with mandatory EnumValues/ValueAsText (non-contiguous codes); test data_access.rs::multi_state_value_discrete_tracks_non_contiguous_enum_values"),
    ...
    (2861, EvidenceStatus::Implemented, "DiscrepancyAlarm (alarms/discrepancy.rs) tracks process-vs-TargetValueNode discrepancy beyond Tolerance persisting past ExpectedTime, register_discrepancy_alarm; test alarms.rs::discrepancy_alarm_activates_after_expected_time_and_clears_at_target"),
    ...
    (2877, EvidenceStatus::Implemented, "ConditionStateMachine::gate_active + LimitAlarm.{on,off}_delay_ms (state_machine.rs, limit.rs::with_delays) defer ActiveState commit until the delay elapses; test alarms.rs::on_delay_and_off_delay_defer_activation_and_deactivation"),
    (2879, EvidenceStatus::Implemented, "ConditionStateMachine::maybe_re_alarm/reset_re_alarm + LimitAlarm::with_re_alarm (state_machine.rs, limit.rs); ReAlarmRepeatCount server-maintained, resets on return to normal per spec text (corrected from task's initial ack-reset assumption); test alarms.rs::re_alarm_time_renotifies_while_active_and_resets_on_return_to_normal"),
    (2881, EvidenceStatus::Implemented, "ConditionStateMachine::recompute_audible_enabled (state_machine.rs) computes AudibleEnabled from active/acked/silenced; AudibleSound modeled as a plain property (AudioDataType has no generated Rust type); Acknowledge now also auto-silences (transitions.rs) per spec; test alarms.rs::audible_enabled_tracks_active_unacked_unsilenced_and_acknowledge_auto_silences"),
    ...
    (2918, EvidenceStatus::Implemented, "alarms/limit.rs:467-468 HasEventSource + test L1487"),
    ...
    (2936, EvidenceStatus::Implemented, "write_node_value (address_space/utils.rs) threads client status/source_timestamp/server_timestamp through to Variable::set_value_range (fixed a real bug: server_timestamp was hardcoded to now()); test write.rs::write_status_code_and_timestamps_round_trip"),
    ...
    (2946, EvidenceStatus::Implemented, "RateOfChangeAlarm (alarms/rate_of_change.rs) non-exclusive path via create_non_exclusive_in_address_space + LimitAlarmKind::RateOfChange; test alarms.rs::rate_of_change_alarm_reports_type_definition_and_activates_on_fast_change"),
    ...
    (2950, EvidenceStatus::Implemented, "Feature 109 CU 2950: simple.rs history_read_raw_modified now honors timestamps_to_return; tests history_data_inmemory.rs distinct_source_server_timestamps_survive_round_trip + sqlite history_update_data.rs distinct_source_server_timestamps_survive_round_trip"),
    (2951, EvidenceStatus::Implemented, "DeviationAlarm (alarms/deviation.rs) exclusive path via create_exclusive_in_address_space + LimitAlarmKind::Deviation; test alarms.rs::deviation_alarm_reports_deviation_type_definition_and_activates_on_setpoint_deviation"),
    ...
    (2969, EvidenceStatus::Implemented, "base_info::create_enum_variable_with_value_as_text/update_enum_value attach a ValueAsText property kept in sync with an enumerated Variable's Value; test base_info.rs::value_as_text_tracks_enumerated_value_changes"),
    ...
    (2988, EvidenceStatus::Implemented, "create_multi_state_discrete_variable (data_access.rs) instantiates MultiStateDiscreteType with mandatory EnumStrings; test data_access.rs::multi_state_discrete_exposes_enum_strings_and_value"),
    (2991, EvidenceStatus::Implemented, "Feature 107: closed as a byproduct of CU 3020's ReadAtTimeDetails implementation -- the Stepped step-hold path (`history/backend.rs`'s `read_at_time` default method) works for any `Variant` type, including structured/non-numeric historized values, verified by a dedicated unit test (`data_history.rs::read_at_time_stepped_node_step_holds_any_variant_type`)."),
    ...
    (3020, EvidenceStatus::Implemented, "Feature 107: OPC-10000-11 section 6.5.5.2 ReadAtTimeDetails. HistoryStorageBackend gains a real `read_at_time` default method (history/backend.rs), backed by a new `read_raw_reverse` default method (nearest-sample-before search, since read_raw_modified alone can't answer that efficiently and the in-memory backend silently ignores return_bounds) overridden on both InMemoryDataHistory and SqliteHistoryBackend. Exact-timestamp matches are Raw; otherwise interpolates per useSimpleBounds (immediate-neighbor vs bounded outward search past Bad-quality points), reusing the Interpolated Aggregate's own ratio-interpolation math. Per-node Stepped configuration selects step-hold vs numeric sloped interpolation. Genuinely supports ContinuationPoint pagination over req_times by index, per the real spec text (\"the standard ContinuationPoint rules apply\"). SimpleNodeManagerImpl::history_read_at_time wired alongside the other 4 history_read_* overrides. 9 backend unit tests plus a real client HistoryRead(ReadAtTimeDetails) end-to-end test."),
    ...
    (3127, EvidenceStatus::Implemented, "base_info::create_option_set_variable instantiates OptionSetType with OptionSetValues/BitMask; test base_info.rs::option_set_exposes_per_bit_values_and_bitmask"),
    ...
    (3142, EvidenceStatus::Implemented, "Feature 109 CU 3142: sample() passes data_encoding through pipeline (subscriptions/mod.rs:1230); test async-opcua-server/tests/monitored_item_data_encoding.rs monitored_item_data_encoding_xml_passed_to_sample_closure"),
    ...
    (3170, EvidenceStatus::Implemented, "Client::register_server2 (async-opcua-client/src/session/client.rs:879) + client-callable discovery-configuration support; tests discovery.rs::register_server2_mdns_config_result_matches_feature_support and :303 over secured_endpoint() (SignAndEncrypt)"),
    ...
    (3194, EvidenceStatus::Implemented, "Feature 109 CU 3194: MaxSelectClauseParameters/MaxWhereClauseParameters live-wired from config (node_manager/memory/core.rs, config/limits.rs); test async-opcua-server/tests/event_filter_tests.rs max_select_where_clause_parameters_are_populated"),
    ...
    (3198, EvidenceStatus::Implemented, "ServerStatusWrapper::schedule_shutdown/estimated_return_time (server_status.rs) + ServerHandle::shutdown_after_with_return_time (server_handle.rs) extend the existing shutdown mechanism; wired core.rs get_attribute; test base_info.rs::estimated_return_time_reflects_scheduled_shutdown_and_is_null_otherwise"),
    ...
    (3201, EvidenceStatus::Implemented, "Feature 109 CU 3201: custom-codegen sample has complete e2e test for all custom EventTypes w/ HasEncoding references; test samples/custom-codegen/tests/encoding_objects_tests.rs custom_event_types_have_encoding_objects"),
    ...
    (3210, EvidenceStatus::Implemented, "Feature 106: fota/file_node.rs's structurally-correct FileType Write Method now has real behavior. fota/file_access.rs wires a real Write handler against the method's NodeId via SimpleNodeManagerImpl::add_method_callback_with_context (register_file_access_methods), performing a real std::fs::File write at the handle's tracked position (OPC-10000-20 section 4.2.5), bounded by MaxByteStringLength, empty/null data a documented no-op, live Size-property update. e2e-proven via fota_file_access_integration.rs (real client -> Call service -> registered handler -> real bytes on disk, independently verified via std::fs::read, then read back byte-for-byte through a separate Open/Read/Close sequence) plus 18 unit tests."),
    ...
    (3224, EvidenceStatus::Implemented, "tests/integration/node_management.rs add_nodes_emits_audit_event, delete_nodes_emits_audit_event, add_references_emits_audit_event, delete_references_emits_audit_event"),
    ...
    (3323, EvidenceStatus::Implemented, "create_y_array_item_variable (data_access.rs) instantiates YArrayItemType with mandatory EURange/EngineeringUnits/Title/AxisScaleType/XAxisDefinition; test data_access.rs::y_array_item_exposes_spectrum_and_x_axis_definition"),
    (3324, EvidenceStatus::Implemented, "create_xy_array_item_variable (data_access.rs) instantiates XYArrayItemType (XVType-valued) with mandatory base Properties + XAxisDefinition; test data_access.rs::xy_array_item_exposes_xv_type_peaks"),
    (3325, EvidenceStatus::Implemented, "create_image_item_variable (data_access.rs) instantiates ImageItemType (2-D) with mandatory base Properties + X/YAxisDefinition; test data_access.rs::image_item_exposes_2d_matrix_and_both_axis_definitions"),
    (3326, EvidenceStatus::Implemented, "create_cube_item_variable (data_access.rs) instantiates CubeItemType (3-D) with mandatory base Properties + X/Y/ZAxisDefinition; test data_access.rs::cube_item_exposes_3d_volume_and_all_three_axis_definitions"),
    (3327, EvidenceStatus::Implemented, "create_nd_dimension_array_item_variable (data_access.rs) instantiates NDimensionArrayItemType with one AxisDefinition per dimension; test data_access.rs::nd_dimension_array_item_exposes_one_axis_definition_per_dimension"),
    ...
    (3539, EvidenceStatus::Implemented, "tests/integration/rbac.rs configure_admin_permissions_match_well_known_role_definition"),
    (3540, EvidenceStatus::Implemented, "tests/integration/rbac.rs authenticated_user_permissions_match_well_known_role_definition"),
    (3541, EvidenceStatus::Implemented, "tests/integration/rbac.rs observer_engineer_supervisor_permissions_match_well_known_role_definitions"),
    (3542, EvidenceStatus::Implemented, "Feature 109 CU 3542: RoleMappingRuleChangedAuditEventType emitted on identity mutation (session/audit.rs dispatch_role_mapping_rule_changed_audit); test async-opcua-server/tests/event_filter_tests.rs add_identity_dispatches_role_mapping_rule_changed_audit_event"),
    (3544, EvidenceStatus::Implemented, "tests/resend_data.rs"),
    ...
    (3546, EvidenceStatus::Implemented, "Feature 109 CU 3546: BaseEventType.local_time now populated from Server_LocalTime source (events/event.rs set_local_time, node_manager/memory/core.rs current_timezone_data); test async-opcua-server/tests/event_filter_tests.rs emitted_event_has_populated_local_time"),
    ...
    (3560, EvidenceStatus::Implemented, "base_info::add_ordered_object attaches HasInterface from each OrderedListType child to IOrderedObjectType (a byproduct of closing CU 2512); test base_info.rs::ordered_list_children_are_ordered_and_interface_conformant"),
    ...
    (3605, EvidenceStatus::Implemented, "Feature 109 CU 3605: MaxNodesPerMethodCall live-wired (node_manager/memory/core.rs:888-892); test async-opcua/tests/integration/methods.rs call_limits"),
    ...
    (3968, EvidenceStatus::Implemented, "Feature 109 CU 3968: dispatch_history_update_audit in session/audit.rs wired at HistoryUpdate handler (session/services/attribute.rs); test async-opcua/tests/integration/write.rs history_update_emits_audit_event"),
    ...
    (3996, EvidenceStatus::Implemented, "base_info::attach_reference_description instantiates ReferenceDescriptionVariableType via HasReferenceDescription, documenting a real Reference's SourceNode/ReferenceType/IsForward/TargetNode (OPC-10000-23 SS5, not Part 3/5); test base_info.rs::reference_description_documents_a_real_reference"),
    ...
    (4237, EvidenceStatus::Implemented, "NonVolatile/Constant bits defined enums.rs:15-19, generic get/set variable.rs:826-838; test write.rs::access_level_ex_non_volatile_and_constant_round_trip"),
    ...
    (4466, EvidenceStatus::Implemented, "alarms/dialog.rs:201 respond2() + tests L424-L445"),
    ...
    (5208, EvidenceStatus::Implemented, "subscriptions/monitored_item.rs:1801 notify_data_value_with_index_range_delivers_sub_value"),
    ...
    (5240, EvidenceStatus::Implemented, "base_info::create_currency_variable attaches a CurrencyUnit property (CurrencyUnitType) to a monetary DataVariable; test base_info.rs::currency_unit_property_reports_iso4217_fields"),
    ...
    (5813, EvidenceStatus::Implemented, "history.rs:26-45 HistoryReadDetails (RawModified/AtTime/Processed/Events/Annotations); attribute.rs:194-229 dispatch; all five sub-modes (RawModified/Processed/Events/Annotations/AtTime, the last closed by feature 107 -- see CU 3020) are functional and tested."),
...
#[cfg(test)]
mod tests {
    use super::{generate_markdown_report, parse_snapshot, EvidenceStatus, AUDIT_TABLE};
...
"#;
    ...
    #[test]
    fn closed_conformance_units_are_implemented_with_current_evidence() {
        let expected = [
            (
                2203,
                "tests/integration/write_service.rs write_extension_object_round_trips_through_read_service",
            ),
            (
                2275,
                "alarms/discrete.rs:199 trip_alarm_activates_returns_to_normal_and_has_trip_alarm_type_definition",
            ),
            (
                2318,
                "subscriptions/monitored_item.rs sanitize_queue_size_clamps_to_server_maximum",
            ),
            (2422, "tests/security_tests.rs audit_over_encrypted_channel"),
            (
                2454,
                "tests/method_call_tests.rs custom_in_memory_node_manager_default_method_call_round_trips_extension_object_argument",
            ),
            (
                2811,
                "alarms/state_machine.rs:537-541 GeneratesEvent→TransitionEventType + test L1564; programs/methods.rs:332-336 GeneratesEvent→ProgramTransitionEventType + test L588",
            ),
            (
                2814,
                "alarms/state_machine.rs:262-308 AvailableStates/Transitions + test L1589; programs/methods.rs:262-308 + test L622",
            ),
            (
                2818,
                "subscriptions/monitored_item.rs sampled_extension_object_reaches_subscription_notification",
            ),
            (
                2918,
                "alarms/limit.rs:467-468 HasEventSource + test L1487",
            ),
            (
                3224,
                "tests/integration/node_management.rs add_nodes_emits_audit_event, delete_nodes_emits_audit_event, add_references_emits_audit_event, delete_references_emits_audit_event",
            ),
            (
                3539,
                "tests/integration/rbac.rs configure_admin_permissions_match_well_known_role_definition",
            ),
            (
                3540,
                "tests/integration/rbac.rs authenticated_user_permissions_match_well_known_role_definition",
            ),
            (
                3541,
                "tests/integration/rbac.rs observer_engineer_supervisor_permissions_match_well_known_role_definitions",
            ),
            (3544, "tests/resend_data.rs"),
            (
                4466,
                "alarms/dialog.rs:201 respond2() + tests L424-L445",
            ),
            (
                5208,
                "subscriptions/monitored_item.rs:1801 notify_data_value_with_index_range_delivers_sub_value",
            ),
        ];

        for (cu_id, expected_evidence) in expected {
            let index = AUDIT_TABLE
                .binary_search_by_key(&cu_id, |(id, _, _)| *id)
                .expect("closed CU must remain in AUDIT_TABLE");
            let (_, status, evidence) = AUDIT_TABLE[index];

            assert_eq!(status, EvidenceStatus::Implemented, "CU {cu_id}");
            assert_eq!(evidence, expected_evidence, "CU {cu_id}");
        }
    }
}
