...
static AUDIT_TABLE: &[(u32, EvidenceStatus, &str)] = &[
    ...
    (3582, EvidenceStatus::Implemented, "Feature 103 (Run 1): fixed the CertificateDirectoryType Pull-model implementation in gds/pull_methods.rs, which previously implemented the wrong (Push-model) methods (GetRejectedList/UpdateCertificate) against fabricated NodeIds -- the same defect class as pre-fix push_methods.rs. CertificateDirectoryType (Part 12 section 7.9.2) exists only in the GDS companion NodeSet2.xml, not the core nodeset, and ships with no pre-instantiated singleton -- unlike ServerConfigurationType. Made async-opcua-server::companion pub and wired its import_gds() into a new opt-in gds::register_gds_pull_methods_from_companion(), gated on the (pre-existing, previously dormant) companion-gds feature; new gds/directory_instance.rs walks CertificateDirectoryType's Mandatory shape and materializes a real 'Directory' object instance (HasTypeDefinition back to the imported companion type) plus its six Mandatory methods and a CertificateGroups/DefaultApplicationGroup/TrustList subtree (built from core namespace-0 CertificateGroupFolderType/CertificateGroupType/TrustListType); NodeIds verified live against the real GDS NodeSet2.xml. Rewrote gds/pull_methods.rs entirely: StartSigningRequest/StartNewKeyPairRequest (new X509::issue_certificate_for_public_key CA-issuance primitive, non-self-signed, for a caller-supplied or freshly generated public key)/FinishRequest/GetCertificateGroups/GetTrustList/GetCertificateStatus, all against the dynamically-resolved NodeIds (no hardcoded namespace index). RevokeCertificate/GetCertificates/CheckRevocationStatus (Optional) deferred -- each needs new ledger/CRL-mutation infrastructure this run didn't build (SourceIssue-adjacent, documented in TODO.md). e2e-proven via gds_pull_companion_integration.rs (real client -> Call service -> registered handler dispatch) plus 10 unit tests in pull_methods/mod.rs and 2 in directory_instance.rs. This investigation also found and fixed three independent, previously-undiscovered bugs in shared node-manager infrastructure that any companion-spec runtime import would have hit: import_companion_xml seeded a disconnected NamespaceMap::default() instead of the address space's real registered namespaces (risking namespace-index collisions); InMemoryNodeManager::owns_node checked a namespace-set snapshot frozen at construction time, never refreshed for namespaces added by a later runtime import (now RwLock-backed with a refresh_namespaces() call site); and CoreNodeManagerImpl::call_builtin_method, which under the default-on subscriptions-standard feature unconditionally short-circuited before consulting the generic method-callback registry for any method outside the core namespace-0 MethodId set -- silently swallowing any custom method callback registered for a companion-spec or other custom-namespace method. Client-side async-opcua-client/src/gds/ has the same fabricated-NodeId defect and needs dynamic Browse/TranslateBrowsePath-based discovery (a real external GDS deployment assigns its own namespace index) -- explicitly out of scope for this run, recorded in TODO.md as Run 2."),
